  ...
  async function loadSeasonsAndEpisodes(showId, seasonSelect, episodeList) {
    try {
      ...
      episodeList.innerHTML =
        '<div class="episode-guide__error">Failed to load episodes. Please try again.</div>';
    }
  }
  ...
  async function loadEpisodesForSeason(showId, seasonNumber, episodeList) {
    try {
      ...
      if (episodes.length === 0) {
        episodeList.innerHTML =
          '<div class="episode-guide__empty">No episodes found for this season.</div>';
        return;
      }
  ...
  function updateAppDataProgress(showId, seasonNumber, watchedEpisodes) {
    ...
    for (const mediaType of mediaTypes) {
      ...
      for (const list of lists) {
        ...
        const showIndex = window.appData[mediaType][list].findIndex(
          (item) => (item.id || item.tmdb_id || item.tmdbId) == showId,
        );
        ...
        if (showIndex !== -1) {
          ...
          window.appData[mediaType][list][showIndex].episodeProgress[seasonNumber] =
            watchedEpisodes;
