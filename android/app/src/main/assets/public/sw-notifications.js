...
const PROD_URL = 'https://flicklet.netlify.app';
...
async function getUserWatchlist() {
  try {
    ...
    const response = await fetch(`${PROD_URL}/.netlify/functions/get-watchlist`);
    if (response.ok) {
...
async function checkShowForUpcomingEpisodes(show) {
  try {
    ...
    const response = await fetch(`${PROD_URL}/.netlify/functions/get-upcoming-episodes?showId=${show.id}`);
