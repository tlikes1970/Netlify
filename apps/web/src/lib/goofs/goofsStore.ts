 ...
 * Update Path: Admin triggers generation via Firebase callable function → Netlify function → Firestore
 * Dependencies: Firestore, localStorage (cache), Netlify function for generation
 *
 * DATA FLOW:
 * - Admin-only job: Admin UI calls Firebase callable function (ingestGoofs)
 * - Firebase function verifies admin status, then calls Netlify function with admin token
 * - Netlify function fetches/transforms data and writes to Firestore (insights collection)
 * - Client reads only from Firestore + local cache (no direct external API calls)
 * - Seed data used as fallback only
 *
