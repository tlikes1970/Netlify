...
export async function fetchGenreContent(mainGenre: string, subGenre: string) {
  ...
  console.log(`📋 Genre IDs: Main=${mainGenreIds.movie.join(',')}, Sub=${subGenreIds.join(',')}`);
  ...
  try {
    ...
    console.log(`✅ Returning ${combined.length} items for ${mainGenre}/${subGenre}`);
...
export async function getTheatersNearLocation(latitude: number, longitude: number, radius: number = 10) {
  try {
    console.log(`🎬 Generating realistic theaters near ${latitude}, ${longitude} within ${radius}km using TMDB data`);
...
async function generateRealisticTheaters(latitude: number, longitude: number, radius: number): Promise<Theater[]> {
  try {
    ...
    const theaterChains = [
      'AMC', 'Regal Cinemas', 'Cinemark', 'Marcus Theatres', 
      'Landmark Theatres', 'Alamo Drafthouse', 'ArcLight Cinemas'
    ];
...
function generateTheaterName(chain: string, index: number): string {
  const suffixes = [
    'Downtown', 'Mall', 'Plaza', 'Center', 'Square', 'Commons',
    'Crossing', 'Station', 'Marketplace', 'Town Center'
  ];
...
async function generateRealisticAddress(latitude: number, longitude: number, index: number): Promise<string> {
  try {
    ...
    if (response.ok) {
      ...
      const streetNames = [
        'Main St', 'Oak Ave', 'Pine St', 'Elm St', 'Maple Ave',
        'Cedar Blvd', 'Park Ave', 'Broadway', 'First St', 'Second Ave',
        'Theater Way', 'Cinema Blvd', 'Movie St', 'Entertainment Ave'
      ];
...
async function generateMovieBasedShowtimes(): Promise<Showtime[]> {
  try {
    ...
    const timeSlots = [
      ...
      '5:00 PM', '6:30 PM', '8:00 PM', '9:30 PM', '11:00 PM'
    ];
    ...
    for (let i = 0; i < numShowtimes; i++) {
      ...
      let availability;
      if (hour24 >= 19) { // Evening (7 PM+)
        availability = Math.random() > 0.25; // 75% chance available
      } else if (hour24 >= 14) { // Afternoon (2 PM+)
...
export async function getShowtimesForMovie(movieId: string, theaterId: string) {
  try {
    console.log(`🎬 Generating realistic showtimes for movie ${movieId} at theater ${theaterId} using TMDB data`);
    
    // Generate realistic showtimes based on TMDB movie data
    const showtimes = await generateMovieBasedShowtimesFromTMDB(movieId);
    
    console.log(`🎭 Generated ${showtimes.length} realistic showtimes for movie ${movieId}`);
    return showtimes;
