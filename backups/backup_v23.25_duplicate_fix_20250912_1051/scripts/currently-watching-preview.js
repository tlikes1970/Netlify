...
(function() {
  ...
  function renderCurrentlyWatchingPreview() {
    ...
    if (!watchingItems || watchingItems.length === 0) {
      ...
      // Also set up a retry for real data (only once to prevent infinite loops)
      if (!window._currentlyWatchingRetryScheduled) {
      ...
      if (watchingList) {
        ...
        if (cards.length > 0) {
          ...
          if (domItems.length > 0) {
            ...
            // CRITICAL: Sync DOM data back to appData to fix persistence issue
            console.log('🔄 Syncing DOM data back to appData...');
            domItems.forEach(item => {
              const category = item.mediaType === 'tv' ? 'tv' : 'movies';
              if (!window.appData[category]) {
                window.appData[category] = { watching: [], wishlist: [], watched: [] };
              }
              if (!window.appData[category].watching) {
                window.appData[category].watching = [];
              }
              
              // Check if item already exists in appData
              const exists = window.appData[category].watching.some(existing => 
                Number(existing.id) === Number(item.id)
              );
              
              if (!exists) {
                console.log('➕ Adding', item.title, 'to appData', category, 'watching');
                window.appData[category].watching.unshift(item);
              }
            });
            
            // Save the synced data
            if (typeof window.saveAppData === 'function') {
              console.log('💾 Saving synced data to storage...');
              window.saveAppData();
            }
