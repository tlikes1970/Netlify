...
export const ingestGoofs = onCall(
  ...
  async (request) => {
    ...
    console.log(
      "[ingestGoofs] Function invoked by admin user:",
      request.auth?.uid
    );
    ...
    if (mode === "bulk") {
      ...
      try {
        ...
        if (firestoreTitles.length > 0) {
          ...
          for (const titleDocument of firestoreTitles) {
            // Skip titles without tmdbId (shouldn't happen due to repository validation, but double-check)
            if (!titleDocument.tmdbId) {
              console.warn(
                `[ingestGoofs] Skipping title "${titleDocument.title}" - missing tmdbId`
              );
              continue;
            }
            ...
            let keywords = null;
            try {
              const titleDocumentReference = database
                .collection("titles")
                .doc(String(titleDocument.tmdbId));
              const titleDocumentSnapshot = await titleDocumentReference.get();
              if (titleDocumentSnapshot.exists) {
                const titleData = titleDocumentSnapshot.data();
                runtimeMins =
                  titleData?.runtimeMinutes ||
                  titleData?.runtimeMins ||
                  titleData?.runtime ||
                  null;
            ...
            titlesList.push({
              tmdbId: titleDocument.tmdbId,
              metadata: {
                tmdbId: titleDocument.tmdbId,
                id: titleDocument.tmdbId,
                title: titleDocument.title,
                mediaType: titleDocument.mediaType,
                genres: titleDocument.genres || [],
                year: titleDocument.year || null,
        ...
        console.log(
          `[ingestGoofs] Found ${titlesList.length} titles to process`
        );
        ...
        const titlesToProcess = TEST_MODE ? titlesList.slice(0, 5) : titlesList;

        if (TEST_MODE) {
          console.log(
            `[ingestGoofs] ⚠️ TEST MODE: Processing only first 5 titles (${titlesToProcess.length} total)`
          );
        }
        ...
        for (
          let index = 0;
          index < titlesToProcess.length;
          index += BATCH_SIZE
        ) {
          const batch = titlesToProcess.slice(index, index + BATCH_SIZE);
          console.log(
            `[ingestGoofs] Processing batch ${Math.floor(index / BATCH_SIZE) + 1}/${Math.ceil(titlesToProcess.length / BATCH_SIZE)} (${batch.length} titles)...`
          );
              ...
              if (response.ok) {
                ...
                console.log(
                  `[ingestGoofs] ✅ Successfully ingested title ${title.tmdbId}: ${title.metadata.title}`,
                  {
                    ...
                    firstInsightPreview:
                      firstInsight?.text?.slice(0, 80) || null,
                ...
                try {
                  ...
                  if (isHtml) {
                    ...
                    const titleMatch = errorText.match(
                      /<title[^>]*>([^<]+)<\/title>/i
                    );
                ...
                if (errors.length === 0) {
                  console.error(
                    `[ingestGoofs] First error details - Status: ${response.status}, URL: ${NETLIFY_FUNCTION_URL}, Response preview: ${errorText.slice(0, 200)}`
                  );
                }
                return {
                  success: false,
