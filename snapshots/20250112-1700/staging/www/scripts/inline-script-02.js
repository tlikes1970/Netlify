      ...
      if (typeof firebase === 'undefined') {
        ...
        try {
          ...
          if (firebase.apps && firebase.apps.length > 0) {
            ...
            // Firebase initialization removed - handled by main initialization in index.html
            // This prevents duplicate initialization and race conditions
            console.warn('Firebase not initialized by main system, using fallback');
            auth = { signInWithPopup: () => Promise.reject(new Error('Firebase not available')) };
            db = { collection: () => ({ doc: () => ({ get: () => Promise.reject(new Error('Firebase not available')) }) }) };
            firebaseInitialized = false;
          }
      ...
      function openModal(title, html, testId = "generic-modal") {
        ...
        const close = () => {
          ...
          if (opener && opener !== document.body) {
            ...
            // Fallback: focus the first focusable element in main content
      ...
      function addToList(item, list) {
        ...
        // Focus management: Return focus to the invoking button or next meaningful element
        const activeElement = document.activeElement;
        if (activeElement && activeElement.closest('.card')) {
          // If focus is on a card element, keep it there
          activeElement.focus();
        } else {
          // Otherwise, focus the first focusable element in the main content
          const focusable = document.querySelector('#main button, #main [tabindex]:not([tabindex="-1"])');
          if (focusable) {
            focusable.focus();
          }
          ...
          const errorRegion = document.querySelector('[data-status-live]');
          if (errorRegion) {
            errorRegion.textContent = `Error: Could not add item to list. ${error.message}`;
          }
          showNotification('Error adding item to list. Please try again.', 'error');
        }
      }
