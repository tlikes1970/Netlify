/**
 * Unified Poster Card Component - KISS Design
 * 
 * Process: Unified Poster Card
...
(function() {
  ...
  console.log('🎴 Unified Poster Card component loaded');

  /**
   * Create unified poster card element following KISS design
   * @param {Object} item - Item data
   * @param {string} section - Section type ('watching', 'wishlist', 'watched', 'discover', 'search')
   * @returns {HTMLElement} Poster card element
   */
  function createUnifiedPosterCard(item, section = 'discover') {
    if (!item) {
      console.log('❌ createUnifiedPosterCard: No item provided');
      return null;
    }

    console.log('🎴 createUnifiedPosterCard called with:', { item, section });

    try {
      const card = document.createElement('article');
      card.className = `unified-poster-card unified-poster-card--${section}`;
      card.dataset.id = item.id || item.tmdb_id || item.tmdbId;
      card.dataset.section = section;
      card.draggable = true; // Enable drag-reorder
      ...
      card.innerHTML = `
        <div class="unified-poster-card__poster" role="button" tabindex="0" aria-label="${title}">
          ${posterUrl ? 
          ...
          <div class="unified-poster-card__placeholder" style="display: ${posterUrl ? 'none' : 'flex'};">
            🎬
          </div>
          ${hasNotes ? '<div class="unified-poster-card__note-indicator" title="Has notes">📝</div>' : ''}
        </div>
        <div class="unified-poster-card__content">
          <h3 class="unified-poster-card__title" title="${title}">${title.toUpperCase()}${year ? ` (${year})` : ''}</h3>
          ${rating ? `<div class="unified-poster-card__rating">★${rating}</div>` : ''}
          ${nextEpisodeInfo}
          ${availability ? `<div class="unified-poster-card__availability">${availability}</div>` : ''}
          <div class="unified-poster-card__actions">
            ${generateSectionActions(section)}
            <button class="unified-poster-card__overflow-btn" aria-label="More actions" aria-expanded="false">
              <span class="unified-poster-card__overflow-dots">•••</span>
            </button>
          </div>
        </div>
      `;
      ...
      // Add event listeners
      setupUnifiedCardEventListeners(card, { item, section });

      return card;
    } catch (error) {
      console.error('❌ Failed to create unified poster card:', error);
      return null;
    }
  }
  ...
  function getNextEpisodeInfo(item, section) {
    ...
    if (nextEpisode && nextEpisode.episode_number && nextEpisode.season_number) {
      ...
      return `
        <div class="unified-poster-card__next-episode">
          <span class="unified-poster-card__next-label">NEXT:</span>
          <span class="unified-poster-card__next-details">S${nextEpisode.season_number}E${nextEpisode.episode_number}${formattedDate ? ` (${formattedDate})` : ''}</span>
        </div>
      `;
    }
    ...
    if (status) {
      ...
      return `
        <div class="unified-poster-card__next-episode">
          <span class="unified-poster-card__next-label">STATUS:</span>
          <span class="unified-poster-card__next-details">${statusText}</span>
        </div>
      `;
    }
  ...
  function generateSectionActions(section) {
    ...
    return sectionActions.map(action => 
      `<button class="unified-poster-card__action-btn btn btn--sm ${action.class}" 
               data-action="move" 
  ...
  /**
   * Set up event listeners for unified card interactions
   * @param {HTMLElement} card - Card element
   * @param {Object} options - Event handler options
   */
  function setupUnifiedCardEventListeners(card, { item, section }) {
    // Poster/title click for details
    const poster = card.querySelector('.unified-poster-card__poster');
    const title = card.querySelector('.unified-poster-card__title');
    ...
    // Action buttons
    const actionButtons = card.querySelectorAll('.unified-poster-card__action-btn');
    ...
    // Overflow menu
    const overflowBtn = card.querySelector('.unified-poster-card__overflow-btn');
  ...
  function setupDragAndDrop(card, item, section) {
    ...
    card.addEventListener('dragstart', (e) => {
      ...
      card.classList.add('unified-poster-card--dragging');
      
      // Add visual feedback
      const container = card.closest('.unified-poster-cards-grid');
      if (container) {
        container.classList.add('unified-poster-cards-grid--reordering');
      }
    });

    card.addEventListener('dragend', (e) => {
      card.classList.remove('unified-poster-card--dragging');
      
      // Remove visual feedback
      const container = card.closest('.unified-poster-cards-grid');
      if (container) {
        container.classList.remove('unified-poster-cards-grid--reordering');
      }
    });
  ...
  function getDragAfterElement(container, y) {
    const draggableElements = [...container.querySelectorAll('.unified-poster-card:not(.unified-poster-card--dragging)')];
  ...
  function updateListOrder(container, draggedItem, section) {
    ...
    const cards = container.querySelectorAll('.unified-poster-card');
  ...
  function createOverflowMenu(item, section) {
    ...
    menu.className = 'unified-overflow-menu';
    ...
    const menuHTML = menuItems
      ...
      .map(menuItem => `
        <button class="unified-overflow-menu__item ${menuItem.destructive ? 'unified-overflow-menu__item--destructive' : ''} ${menuItem.pro ? 'unified-overflow-menu__item--pro' : ''}" 
                data-action="${menuItem.action}">
          <span class="unified-overflow-menu__icon">${menuItem.icon}</span>
          <span class="unified-overflow-menu__label">${menuItem.label}</span>
          ${menuItem.pro ? '<span class="unified-overflow-menu__pro-badge">PRO</span>' : ''}
        </button>
      `).join('');
    ...
    menu.addEventListener('click', (e) => {
      e.stopPropagation();
      const button = e.target.closest('.unified-overflow-menu__item');
      if (button) {
  ...
  function createMobileBottomSheet(item, section) {
    ...
    bottomSheet.className = 'unified-mobile-bottom-sheet';
    
    const menuItems = getSectionMenuItems(item, section);
    const availableItems = menuItems.filter(item => item.available);
    
    bottomSheet.innerHTML = `
      <div class="unified-mobile-bottom-sheet__backdrop"></div>
      <div class="unified-mobile-bottom-sheet__content">
        <div class="unified-mobile-bottom-sheet__handle"></div>
        <div class="unified-mobile-bottom-sheet__header">
          <h3 class="unified-mobile-bottom-sheet__title">${item.title || item.name}</h3>
          <button class="unified-mobile-bottom-sheet__close">×</button>
        </div>
        <div class="unified-mobile-bottom-sheet__body">
          ${availableItems.map(menuItem => `
            <button class="unified-mobile-bottom-sheet__item ${menuItem.destructive ? 'unified-mobile-bottom-sheet__item--destructive' : ''} ${menuItem.pro ? 'unified-mobile-bottom-sheet__item--pro' : ''}" 
                    data-action="${menuItem.action}">
              <span class="unified-mobile-bottom-sheet__icon">${menuItem.icon}</span>
              <span class="unified-mobile-bottom-sheet__label">${menuItem.label}</span>
              ${menuItem.pro ? '<span class="unified-mobile-bottom-sheet__pro-badge">PRO</span>' : ''}
            </button>
          `).join('')}
        </div>
      </div>
    ...
    // Add event listeners
    const backdrop = bottomSheet.querySelector('.unified-mobile-bottom-sheet__backdrop');
    const closeBtn = bottomSheet.querySelector('.unified-mobile-bottom-sheet__close');
    const items = bottomSheet.querySelectorAll('.unified-mobile-bottom-sheet__item');
    
    const closeBottomSheet = () => {
      bottomSheet.classList.add('unified-mobile-bottom-sheet--closing');
      setTimeout(() => {
        bottomSheet.remove();
      }, 300);
    };
    ...
    // Animate in
    setTimeout(() => {
      bottomSheet.classList.add('unified-mobile-bottom-sheet--visible');
    }, 10);
  }
  ...
  function handleAddItem(item, toSection) {
    try {
      ...
      if (targetContainer && targetContainer.closest('.tab-section').style.display !== 'none') {
        const newCard = createUnifiedPosterCard(item, toSection);
        if (newCard) {
  ...
  function rollbackUIChanges(item, fromSection, toSection) {
    ...
    if (sourceContainer) {
      const card = createUnifiedPosterCard(item, fromSection);
      if (card) {
  ...
  function closeAllOverflowMenus() {
    const menus = document.querySelectorAll('.unified-overflow-menu');
    menus.forEach(menu => menu.remove());
    
    const bottomSheets = document.querySelectorAll('.unified-mobile-bottom-sheet');
    bottomSheets.forEach(sheet => sheet.remove());
    
    const cards = document.querySelectorAll('.unified-poster-card[data-overflow-open="true"]');
    cards.forEach(card => {
  ...
  // Expose createUnifiedPosterCard globally
  window.createUnifiedPosterCard = createUnifiedPosterCard;
  
  // Also expose as createPosterCard for backward compatibility
  window.createPosterCard = createUnifiedPosterCard;
