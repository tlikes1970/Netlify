...
fs.writeFileSync(path.join(OUT_DIR, 'cdn-compat.txt'), cdnMatches.map(x=>`${x.line}: ${x.text}`).join('\n'));
fs.writeFileSync(path.join(OUT_DIR, 'blocking-scripts.txt'), blocking.map(x=>`${x.line}: ${x.text}`).join('\n'));
fs.writeFileSync(path.join(OUT_DIR, 'inline-scripts.txt'), inline.map(x=>`${x.line}: ${x.text}`).join('\n'));

const summary = { cdnCompat: cdnMatches.length, blockingExternal: blocking.length, inlineScripts: inline.length };
