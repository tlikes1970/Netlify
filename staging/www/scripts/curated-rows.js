  ...
  function renderCuratedCard(item){
    ...
    return `
      <article class="curated-card" data-id="${m.id}" data-title="${m.title}" ${m.rating?`data-rating="${m.rating}"`:''}>
        <div class="poster">
          ${m.poster ? `<img src="${m.poster}" ${srcset ? `srcset="${srcset}"` : ''} sizes="(max-width: 480px) 148px, 200px" alt="${m.title} poster" loading="lazy">` : `<div class="poster placeholder" aria-hidden="true"></div>`}
        </div>
