...
(function(){
  ...
  function showProRequired() {
    if (window.Toast && window.Toast.show) {
      ...
      window.showNotification('CSV export is a Pro feature. Upgrade to Pro to export your data.', 'info');
    }
  }
  ...
  function exportToCSV() {
    ...
    if (!data || data.length === 0) {
      if (window.Toast && window.Toast.show) {
        window.Toast.show('No data to export. Add some shows or movies to your lists first.', 'info');
      }
      return false;
    }
