...
(function(){
  ...
  function showNext(){
    ...
    if (el.dataset.type !== 'error'){ // errors shouldn't auto-dismiss
  ...
  // Public API
  window.Notify = {
    toast(message, opts){ enqueueToast(message, 'info', opts); },
    success(message, opts){ enqueueToast(message, 'success', opts); },
    info(message, opts){ enqueueToast(message, 'info', opts); },
    error(message, opts){ enqueueToast(message, 'error', Object.assign({ duration: 0 }, opts)); }, // no auto-dismiss
    banner(message, type='info', opts){ setBanner(message, type, opts); },
    clearBanner
  };
})();
