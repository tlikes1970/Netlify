...
(function(){
  ...
  function createEnhancedQuoteDeck() {
    ...
    const categorized = {
      motivational: quotes.filter(q => 
        q.includes('best way') || q.includes('future') || q.includes('great work') ||
        q.includes('believe') || q.includes('dreams') || q.includes('light')
      ),
      sarcastic: quotes.filter(q => 
        q.includes('Netflix') || q.includes('binge') || q.includes('watchlist') ||
        q.includes('addicted') || q.includes('problem') || q.includes('avoiding')
      ),
      fun: quotes.filter(q => 
        q.includes('🎬') || q.includes('🍿') || q.includes('📺') ||
        q.includes('🎭') || q.includes('🎨') || q.includes('🎵')
      ),
      philosophical: quotes.filter(q => 
        q.includes('think') || q.includes('know') || q.includes('life') ||
        q.includes('truth') || q.includes('dream') || q.includes('journey')
      )
    };
  ...
  function startQuoteRotation(intervalMs = 15000) { // 15 seconds default
    if (rotationInterval) {
