...
async function runLighthouse() {
  ...
  const baseUrl = 'http://localhost:64213';
...
async function runAxe() {
  ...
  try {
    await execAsync(`npx @axe-core/cli http://localhost:64213 --save ${path.join(reportsDir, 'axe', 'axe.json')}`);
...
async function runCSSCoverage() {
  ...
  try {
    ...
    await page.goto('http://localhost:64213', { waitUntil: 'networkidle0' });
...
async function main() {
  ...
  try {
    await execAsync('curl -s http://localhost:64213 > /dev/null');
    console.log('✅ Server detected at http://localhost:64213');
  } catch (error) {
    console.log('❌ No server detected at http://localhost:64213');
