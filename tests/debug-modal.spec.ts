...
test('Debug login modal structure', async ({ page }) => {
  ...
  if (isModalVisible) {
    ...
    const closeElements = page.locator(
      '.modal-backdrop [class*="close"], .modal-backdrop [id*="close"], .modal-backdrop [data-action*="close"]',
    );
    ...
    const modalContent = page.locator(
      '.modal-backdrop .modal-content, .modal-backdrop .modal, .modal-backdrop [class*="modal"]',
    );
