...
(function() {
  ...
  async function cleanupFirebaseData() {
    try {
      ...
      console.log('📊 Current document size:', Math.round(JSON.stringify(currentData).length / 1024), 'KB');
      ...
      const cleanedData = {
        ...
        watchlists: {
          tv: {
            watching: (currentData.watchlists?.tv?.watching || []).map(item => cleanItem(item, 'watching')),
            wishlist: (currentData.watchlists?.tv?.wishlist || []).map(item => cleanItem(item, 'wishlist')),
            watched: (currentData.watchlists?.tv?.watched || []).map(item => cleanItem(item, 'watched'))
          },
          movies: {
            watching: (currentData.watchlists?.movies?.watching || []).map(item => cleanItem(item, 'watching')),
            wishlist: (currentData.watchlists?.movies?.wishlist || []).map(item => cleanItem(item, 'wishlist')),
            watched: (currentData.watchlists?.movies?.watched || []).map(item => cleanItem(item, 'watched'))
          }
        },
      ...
      Object.keys(cleanedData.watchlists).forEach(mediaType => {
        Object.keys(cleanedData.watchlists[mediaType]).forEach(listType => {
          cleanedData.watchlists[mediaType][listType] = cleanedData.watchlists[mediaType][listType].filter(item => item !== null);
        });
      });
