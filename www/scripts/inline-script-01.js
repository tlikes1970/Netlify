      ...
      async function addItemToList(tmdbId, listName, source = 'ui') {
        FlickletDebug.info('➕ addItemToList called:', { tmdbId, listName, source });
        ...
        try {
          ...
          // Call the real addToListFromCache function from inline-script-02.js
          FlickletDebug.info('🔄 Calling addToListFromCache function');
          if (typeof window.addToListFromCache === 'function') {
            window.addToListFromCache(tmdbId, listName);
          } else {
            throw new Error('addToListFromCache function not available');
          }
          
        } catch (e) {
          FlickletDebug.error('addItemToList failed', e);
      ...
      // Legacy addToListFromCache function removed - using the real implementation from inline-script-02.js
      window.addItemToList = addItemToList;

      // NEUTERED: Legacy add handler - now handled by centralized-add-handler.js
      ...
      //     FlickletDebug.info('➕ Add button clicked (data-action):', { id, list, element: el });
      //     if (id && list) {
      //       ev.preventDefault();
      //       ev.stopPropagation();
      //       FlickletDebug.info('➕ Centralized add handler called:', { id, list });
      ...
      FlickletDebug.info('✅ Centralized add delegation handler set up');
      ...
      function initFlickWord() {
        ...
        function updateFlickWordStats(date, won, guesses) {
          FlickletDebug.info('🔍 Updating FlickWord stats:', { date, won, guesses });
          ...
          FlickletDebug.info('🔍 New stats:', stats);
        ...
        function checkForNewResults() {
          ...
          FlickletDebug.info('🔍 Checking for new results:', { today, lastPlay, results });
          
          // Check all dates in results, not just today
          for (const [date, result] of Object.entries(results)) {
            if (lastPlay !== date) {
              FlickletDebug.info('🔍 Found unprocessed result for date:', date, result);
      ...
      if (!window.FlickletApp) {
        FlickletDebug.info('🚀 Creating FlickletApp object (fallback system)');
        window.FlickletApp = {
        // Centralized state
        currentUser: null,
        currentTab: 'home',
        ...
        // Core methods
        init() {
          FlickletDebug.info('🚀 Initializing Flicklet App...');
          
          // Check if the new system is available - if so, don't initialize the old system
          if (window.FlickletApp && window.FlickletApp !== this) {
            FlickletDebug.info('🔄 New FlickletApp detected, skipping old system initialization');
            return;
          }
          
          // Check if this is a class-based FlickletApp (new system)
          if (window.FlickletApp && typeof window.FlickletApp === 'function') {
            FlickletDebug.info('🔄 Class-based FlickletApp detected, skipping object-based initialization');
            return;
          }
          
          // Wait for DOM to be fully ready
          FlickletDebug.info('⏰ Setting up setTimeout for DOM ready...');
          setTimeout(() => {
            FlickletDebug.info('⏰ DOM ready timeout completed, setting up app...');
            this.loadData();
            this.applyTheme();
            this.applyLanguage();
            this.applyMardiGras(); // Apply Mardi Gras state
            // initFirebase() called in FlickletApp.init() - no need to call again
            FlickletDebug.info('🔧 About to call setupEventListeners...');
            this.setupEventListeners();
            this.updateUI();
            
            // Call existing initialization functions for compatibility
            if (typeof loadAppData === 'function') {
              FlickletDebug.info('🔄 Calling existing loadAppData');
              loadAppData();
            }
            if (typeof updateUI === 'function') {
              FlickletDebug.info('🔄 Calling existing updateUI');
              updateUI();
            }
            if (typeof switchToTab === 'function') {
              FlickletDebug.info('🔄 Calling existing switchToTab');
              switchToTab('home');
            }
            ...
            if (typeof start === 'function') {
              FlickletDebug.info('🔄 Calling existing start function');
              start();
            }
            
            // Run global initialization after centralized system is ready
            setTimeout(() => {
              FlickletDebug.info('🌍 Running global initialization after centralized system...');
              FlickletDebug.info('🌍 this.runGlobalInitialization type:', typeof this.runGlobalInitialization);
              if (typeof this.runGlobalInitialization === 'function') {
                this.runGlobalInitialization();
              } else {
                FlickletDebug.error('❌ runGlobalInitialization method not available');
                // Fallback: set up search controls directly
                FlickletDebug.info('🔍 Setting up search controls (fallback)...');
                const searchBtn = document.getElementById("searchBtn");
                if (searchBtn) {
                  FlickletDebug.info('✅ Search button found (fallback), binding performSearch');
                  searchBtn.onclick = () => {
                    FlickletDebug.info('🔍 Search button clicked (fallback), calling performSearch');
                    if (typeof window.performSearch === 'function') {
                      window.performSearch();
                    } else {
                      FlickletDebug.error('❌ performSearch function not available');
                    }
                  };
                } else {
                  FlickletDebug.warn('❌ Search button not found (fallback)');
                }
              }
            }, 500);
            
            FlickletDebug.info('✅ App initialized successfully');
            
                    // Initialize the left-side username display if username exists
        if (this.appData?.settings?.displayName) {
          FlickletDebug.info('🆕 Initializing left-side username display');
        ...
        checkAndPromptLogin() {
          ...
          FlickletDebug.info('🔍 Checking login prompt conditions:');
          FlickletDebug.info('  - hasBeenPrompted:', hasBeenPrompted);
          FlickletDebug.info('  - hasData:', hasData);
          FlickletDebug.info('  - isAuthenticated:', isAuthenticated);
          FlickletDebug.info('  - currentUser:', this.currentUser);
          FlickletDebug.info('  - showSignInModal available:', typeof showSignInModal === 'function');
          
          // Don't show login prompt if user is already authenticated
          if (isAuthenticated) {
            FlickletDebug.info('✅ User is already authenticated, no login prompt needed');
            return;
          }
          
          // If no login prompt has been shown before and no user data exists, show login modal
          if (!hasBeenPrompted && !hasData) {
            FlickletDebug.info('🆕 New user detected, showing login prompt');
            ...
            setTimeout(() => {
              FlickletDebug.info('⏰ Timeout completed, attempting to show login modal');
              if (window.FlickletAppInstance && typeof window.FlickletAppInstance.showSignInModal === 'function') {
                FlickletDebug.info('✅ FlickletAppInstance.showSignInModal function available, calling it');
                window.FlickletAppInstance.showSignInModal();
              } else if (window.FlickletApp && typeof window.FlickletApp.showSignInModal === 'function') {
                FlickletDebug.info('✅ FlickletApp.showSignInModal function available, calling it');
                window.FlickletApp.showSignInModal();
              } else {
                FlickletDebug.error('❌ FlickletApp.showSignInModal function not available');
              }
            }, 1000); // Increased timeout to ensure functions are loaded
          } else if (hasBeenPrompted && !hasData) {
            FlickletDebug.info('👤 User has been prompted before but no data exists');
          } else {
            FlickletDebug.info('✅ User has existing data or has been prompted');
          }
        },
        ...
        loadData() {
          try {
            ...
            if (saved) {
              ...
              FlickletDebug.info('💾 Data loaded from centralized localStorage');
            } else {
              ...
              if (oldSaved) {
                ...
                FlickletDebug.info('💾 Data loaded from legacy localStorage including TV/movies data');
              }
            }
          } catch (error) {
            FlickletDebug.error('❌ Failed to load data:', error);
          }
        },
          ...
          try {
            ...
            if (this.currentUser && typeof firebase !== 'undefined' && firebase.firestore) {
              ...
              ref.get().then(async (snap) => {
                ...
                FlickletDebug.info('🔥 Saving to Firebase with safe merge:', payload);
                await ref.set(payload, { merge: true });
                FlickletDebug.info('💾 Data saved to Firebase with safe merge successfully');
              }).catch((error) => {
                FlickletDebug.error('❌ Failed to save to Firebase with safe merge:', error);
              });
            } else {
              FlickletDebug.warn('⚠️ Cannot save to Firebase - user not logged in or Firebase not available');
            }
            
            FlickletDebug.info('💾 Data saved to both storage formats');
          } catch (error) {
            FlickletDebug.error('❌ Failed to save data:', error);
            this.showNotification('Failed to save data', 'error');
          }
        },

        runGlobalInitialization() {
          FlickletDebug.info('🌍 Running global initialization...');
          
          // Run the global initialization code that was in the fallback sequence
          if (typeof loadAppData === 'function') {
            FlickletDebug.info('🔄 Calling loadAppData');
            loadAppData();
          ...
          if (typeof loadGenres === 'function') {
            FlickletDebug.info('🔄 Calling loadGenres');
            loadGenres();
          }
          ...
          FlickletDebug.info('🔍 Search controls handled by app.js - skipping inline setup');
          
          // STEP 3.2c — Attach this delegate ONCE and add de-dup guards
          if (!window.__inline01ActionsBound) {
            window.__inline01ActionsBound = true;
            FlickletDebug.info('🎯 Setting up event delegation for card actions (inline-01)…');
          } else {
            FlickletDebug.info('↩︎ Delegate already bound — skipping rebind');
            return;
          }
          ...
          document.addEventListener("click", (e) => {
            ...
            FlickletDebug.info(`🎯 Action detected: action=${action}, id=${id}, list=${list}, mediaType=${mediaType}`);
            ...
            if (action === "addFromCache") {
              ...
              FlickletDebug.info('➕ Calling addToListFromCache');
              addToListFromCache(id, list);
            } else if (action === "notInterested") {
              FlickletDebug.info('🚫 Calling markAsNotInterested');
        ...
        showSignOutModal() {
          ...
          modalContent.innerHTML = `
            <h3 class="heading-subtle" style="margin: 0 0 16px 0; font-size: 18px;">Account</h3>
            <p class="meta-muted" style="margin: 0 0 20px 0; font-size: 14px;">Signed in as <strong>${displayName}</strong></p>
            <p class="meta-muted" style="margin: 0 0 20px 0; font-size: 14px;">Email: ${email}</p>
        ...
        showNuclearOptionModal() {
          ...
          modalContent.innerHTML = `
            <h3 class="heading-subtle" style="margin: 0 0 16px 0;">🗑️ Nuclear Option</h3>
            <p class="meta-muted" style="margin: 0 0 20px 0; line-height: 1.5;">
    ...
    try {
      ...
      if (settingsSection) {
        ...
        card.innerHTML = `
          ...
          <p class="meta-muted" style="margin: 0 0 15px 0; font-size: 0.9rem; line-height: 1.4;">
    ...
    try {
      ...
      card.innerHTML = `
        ...
        <div class="settings-block ${disabled}" style="display: flex; flex-direction: column; gap: 15px;">
          ...
          ${IS_PRO ? '' : '<div class="meta-muted" role="note">PRO subscription required to use advanced notifications.</div>'}
        </div>
      `;
...
// === MP-ThemePacks T0 (removed) ===
// Theme packs functionality removed - using flicklet-theme system instead
