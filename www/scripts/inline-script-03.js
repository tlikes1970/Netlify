...
document.addEventListener(
  ...
  (e) => {
    ...
    FlickletDebug.info(
      ...
      document.querySelectorAll('.settings-tabs button').length,
    );
    ...
    if (btn.dataset.action === 'track-episodes') {
      ...
      console.log(
        ...
        localStorage.getItem('flicklet:episodeTracking:enabled'),
      );
    ...
    if (btn.dataset.action === 'share-lists') {
      ...
      if (typeof window.openShareSelectionModal === 'function') {
        ...
        console.log(
          '🔗 Available functions:',
          Object.keys(window).filter((k) => k.includes('share')),
        );
      }
      return;
    }
