...
export function initializeFlickWordModal() {
  ...
  function openFlickWordModal() {
    ...
    if (modal && frame) {
      ...
      console.log('🎯 Close button styles:', closeBtn ? window.getComputedStyle(closeBtn) : 'Not found');
  ...
  function updateStatsDisplay() {
    console.log('🎯 Updating stats display after modal close... (attempt', statsUpdateRetryCount + 1, ')');
    ...
    console.log('🎯 All elements with data-testid:', document.querySelectorAll('[data-testid]').length);
    console.log('🎯 All .stats-card elements:', document.querySelectorAll('.stats-card').length);
    console.log('🎯 All elements with "stats" in class:', document.querySelectorAll('[class*="stats"]').length);
    console.log('🎯 All elements with "flickword" in class:', document.querySelectorAll('[class*="flickword"]').length);
    
    // Log the actual elements with "stats" in class
    const statsElements = document.querySelectorAll('[class*="stats"]');
    console.log('🎯 Stats elements found:', Array.from(statsElements).map(el => ({
      tagName: el.tagName,
    ...
    console.log('🎯 Elements with just numbers:', numberElements.map(el => ({
      tagName: el.tagName,
      className: el.className,
      textContent: el.textContent,
      parent: el.parentElement?.tagName + '.' + el.parentElement?.className,
      parentText: el.parentElement?.textContent?.substring(0, 50),
      grandparent: el.parentElement?.parentElement?.tagName + '.' + el.parentElement?.parentElement?.className
    })));
    ...
    const textElements = Array.from(allTextElements).filter(el => {
      ...
      return text && (text.includes('streak') || text.includes('best') || text.includes('win') || text.includes('games'));
    });
    console.log('🎯 Elements with stats-related text:', textElements.map(el => ({
      tagName: el.tagName,
      className: el.className,
    ...
    console.log('🎯 Potential stats display elements:', potentialStatsElements.map(el => ({
      tagName: el.tagName,
      className: el.className,
      textContent: el.textContent,
      parent: el.parentElement?.tagName + '.' + el.parentElement?.className,
    ...
    statLabels.forEach((label, index) => {
      ...
      const valueElement = label.nextElementSibling || label.parentElement?.querySelector('.stat-value, .stat-number, [class*="value"], [class*="number"]');
      if (valueElement) {
    ...
    const flickwordTab = document.querySelector('[data-tab="flickword"]') || 
    ...
    if (flickwordTab) {
      ...
      console.log('🎯 Numbers in FlickWord tab:', flickwordNumberElements.map(el => ({
        tagName: el.tagName,
        className: el.className,
        textContent: el.textContent,
        parent: el.parentElement?.tagName + '.' + el.parentElement?.className
    ...
    const totalGamesElement = document.querySelector('[data-testid="stats-total-games"]') || 
    ...
    if (!streakElement && !bestElement && !winElement && !totalGamesElement && statsUpdateRetryCount < MAX_STATS_UPDATE_RETRIES) {
      statsUpdateRetryCount++;
      console.log('🎯 No stats elements found, trying again in 500ms... (retry', statsUpdateRetryCount, 'of', MAX_STATS_UPDATE_RETRIES, ')');
      setTimeout(() => {
        updateStatsDisplay();
      }, 500);
  ...
  function handleGameResult(result) {
    ...
    try {
      // Load current stats
      const rawStats = JSON.parse(localStorage.getItem('flickword:stats') || 
