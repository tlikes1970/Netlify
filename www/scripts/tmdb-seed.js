...
(function () {
  const API_KEY =
    (typeof window !== 'undefined' && window.__TMDB_API_KEY__) ||
  ...
  console.log('🌱 TMDB Seeder: Starting...', {
  ...
  function touchStamp() {
    try {
      localStorage.setItem(STAMP_KEY, JSON.stringify({ ts: Date.now() }));
    } catch (_) {}
  }
  ...
  async function seed(lang = 'en-US') {
    ...
    try {
      ...
      const [tv, mv] = await Promise.all([
        fetchJSON(
          `https://api.themoviedb.org/3/trending/tv/week?api_key=${API_KEY}&language=${lang}`,
        ),
        fetchJSON(
          `https://api.themoviedb.org/3/trending/movie/week?api_key=${API_KEY}&language=${lang}`,
        ),
      ]);

      console.log('🌱 Seeder: TMDB response:', {
        tv: tv?.results?.length || 0,
        mv: mv?.results?.length || 0,
      });
