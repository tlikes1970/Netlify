...
(function() {
      ...
      <div class="settings-subsection">
        ...
        <div class="settings-preview">
          ...
          <div class="preview-rows">
            ${[0, 1].map(i => {
              const preset = selected[i] ? (window.ROW_PRESETS || []).find(p => p.key === selected[i].key) : null;
              return `
    ...
    try {
      ...
      if (window.setMyRows) {
        ...
        if (success) {
          ...
          try {
            window.dispatchEvent(new CustomEvent('personalized:updated', { detail: { a: rows[0], b: rows[1] }}));
          } catch(e){}
  ...
  function handleClearMyRows() {
    if (confirm(window.t ? window.t('settings.clear_confirm') : 'Are you sure you want to clear all rows?')) {
      try {
    ...
    selects.forEach((select, index) => {
      ...
      if (contentSpan) {
        contentSpan.textContent = preset 
          ? (window.t ? window.t(preset.labelKey) : preset.labelKey)
          : (window.t ? window.t('settings.none') : 'None');
      }
    });
  }
